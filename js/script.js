...
var glossary_terms = {
  "Away from Brownfields":
    "Indicates the degree to which the land is inclined, affecting construction and drainage.",
  "Buildable Soil":
    "Refers to soil with properties conducive to agriculture or construction.",
  "Away from Flood Zones":
    "Areas less likely to experience flooding, reducing risk of water damage.",
  "Proximity to Hospitals":
    "Distance from medical facilities, affecting emergency response times.",
  "Proximity to Police Stations":
    "Distance from law enforcement, affecting crime rates and safety.",
  "Proximity to Roads":
    "Distance from roads, affecting accessibility and noise pollution.",
  "Proximity to Schools":
    "Distance from educational institutions, affecting property values and child safety.",
  "Public Sewer": "Availability of immediate access to public sewer systems.",
  "Low Grade Slope":
    "Indicates the degree to which the land is flat and suitable for construction.",
  "Away from Wetlands":
    "Distance from wetlands, affecting environmental impact and land use.",
  "Appropriate Land Use":
    "Refers to the suitability of the land for specific purposes such as residential, commercial, or industrial.",
};
...
function combineGlossaryAndSliderValues() {
  var combinedData = Object.entries(glossary_terms).reduce(
...
class OpacityControl extends L.Control {
  ...
  onAdd(map) {
    var container = L.DomUtil.create(
      "div",
      "leaflet-bar leaflet-control leaflet-control-custom opacity-control"
    );
    container.innerHTML = `
...
function createTileLayer(
  ...
  // Default values for minZoom, maxZoom, and ext
...
async function addLayerFromGeoJSON(dataPath, iconName) {
  ...
  return L.geoJSON(data, {
    ...
    pointToLayer: (feature, latlng) =>
      ...
      }) // Create a marker with the given icon
...
function getPopupContent(iconName, feature) {
  // Function to get the popup content based on the icon name and feature
  const props = feature.properties; // Get the properties of the feature
  switch (
    iconName // Check the icon name
  ) {
...
function getColor(gridcode) {
  // Function to get the color based on the gridcode
  return gridcode >= 0 && gridcode <= 10
    ? `hsl(${(gridcode / 10) * 120}, 100%, 50%)`
    : "gray"; // Return a color based on the gridcode
}
...
async function addRasterLayer() {
  ...
  let rasterLayer = L.geoJSON(data, {
    // Create a GeoJSON layer with the data
  ...
  map.addControl(
    new OpacityControl(rasterLayer, {
      position: "topright",
    })
...
function updateFeatureLayerVisibility() {
  ...
  if (currentZoom >= zoomThreshold) {
    // If the current zoom level is greater than or equal to the threshold
    if (!map.hasLayer(featureLayer)) {
      // If the map does not have the feature layer
      featureLayer.addTo(map); // Add the feature layer to the map
    }
  } else {
    if (map.hasLayer(featureLayer)) {
      // If the map has the feature layer
...
(async () => {
  const layers = await Promise.all([
    // Load multiple layers asynchronously
  ...
  L.control
    ...
    .addTo(map); // Add the layers control to the map
})();
...
function updateInfoBox(gridcode) {
  ...
  if (infoBox) {
    // If the info box element exists
...
class ReturnToExtentControl extends L.Control {
  // Custom control for returning to the main extent
  constructor(options = {}) {
